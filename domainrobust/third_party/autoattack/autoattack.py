...
from domainrobust.third_party.autoattack.state import EvaluationState


class AutoAttack():
    def __init__(self, model, norm='Linf', eps=.3, num_class=10, seed=None, verbose=True,
        ...
        if not self.is_tf_model:
            ...
            self.apgd_targeted = APGDAttack_targeted(self.model, n_restarts=1, n_iter=100, verbose=False,
                ...
                logger=self.logger, n_target_classes=num_class)
            ...
            self.apgd_targeted = APGDAttack_targeted(self.model, n_restarts=1, n_iter=100, verbose=False,
                eps=self.epsilon, norm=self.norm, eot_iter=1, rho=.75, seed=self.seed, device=self.device,
                is_tf_model=True, logger=self.logger, n_target_classes=num_class)
    
        if version in ['standard', 'plus', 'rand']:
            self.set_version(n_target_classes=num_class, version=version)
    ...
    def set_version(self, n_target_classes=9, version='standard'):
        if self.verbose:
        ...
        if version == 'standard':
            ...
            if self.norm in ['Linf', 'L2']:
                ...
                self.apgd_targeted.n_target_classes = n_target_classes - 1
            ...
            self.fab.n_target_classes = n_target_classes - 1
