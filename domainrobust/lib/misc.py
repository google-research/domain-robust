...
from domainrobust.third_party.autoattack.autoattack import AutoAttack
...
def pgd_accuracy(network, loader, eps, atk_lr, atk_iter, device, restart = 1):
    correct = 0
    total = 0
    ...
    for x, y in loader:
        ...
        for _ in range(restart):
            x_adv = projected_gradient_descent(
                model_featurizer=network.featurizer,
                model_classifier=network.classifier,
            ...
            predict = network.predict(x_adv)
            if predict.size(1) == 1:
                p.append((predict.gt(0).eq(y).float() * batch_weights.view(-1, 1)).type(torch.int64))
            else:
                p.append((predict.argmax(1).eq(y).float() * batch_weights).type(torch.int64))
        result = p[0]
...
def autoattack_accuracy(network, loader, eps, device):
    correct = 0
    total = 0
    ...
    for x, y in loader:
        x = x.to(device)
        y = y.to(device)
        adversary = AutoAttack(network, norm='Linf', eps = eps, num_class = network.predict(x).shape[1], version='standard')
        x_adv = adversary.run_standard_evaluation(x, y, bs=x.shape[0])
        p = network.predict(x_adv)
        batch_weights = torch.ones(len(x)).to(device)
        if p.size(1) == 1:
            correct += (p.gt(0).eq(y).float() * batch_weights.view(-1, 1)).sum().item()
        else:
